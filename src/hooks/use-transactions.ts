...
import { getActivePeriodKey, shiftPeriodKey, getPeriodRange } from "@/lib/period";
...
function getPeriodDates(
  ...
  // Batas periode mengikuti siklus gajian (lib/period.ts), zona Asia/Jakarta
  if (period === "this-month" || period === "last-month") {
    const key =
      period === "this-month"
        ? getActivePeriodKey()
        : shiftPeriodKey(getActivePeriodKey(), -1);
